...
type (
	userAuthModel interface {
		Insert(ctx context.Context, data *UserAuth, opts ...Option) (sql.Result, error)
		...
		Update(ctx context.Context, data *UserAuth, opts ...Option) error
...
func (m *defaultUserAuthModel) Trans(ctx context.Context, fn func(context.Context, sqlx.Session) error) error {
	return m.TransactCtx(ctx, fn)
}

func (m *defaultUserAuthModel) Delete(ctx context.Context, id int64, opts ...Option) error {
	// 1. 解析 Options
	var o options
...
func (m *defaultUserAuthModel) Insert(ctx context.Context, data *UserAuth, opts ...Option) (sql.Result, error) {
	// 1. 解析 Options
	var o options
	for _, opt := range opts {
		opt(&o)
	...
	ret, err := m.ExecCtx(ctx, func(ctx context.Context, conn sqlx.SqlConn) (result sql.Result, err error) {
		...
		if o.Session != nil {
			return o.Session.ExecCtx(ctx, query, data.DeleteTime, data.DelState, data.Version, data.UserId, data.AuthKey, data.AuthType)
		}
...
func (m *defaultUserAuthModel) Update(ctx context.Context, newData *UserAuth, opts ...Option) error {
	// 1. 解析 Options
	var o options
	for _, opt := range opts {
		opt(&o)
	...
	_, err = m.ExecCtx(ctx, func(ctx context.Context, conn sqlx.SqlConn) (result sql.Result, err error) {
		...
		if o.Session != nil {
			return o.Session.ExecCtx(ctx, query, newData.DeleteTime, newData.DelState, newData.Version, newData.UserId, newData.AuthKey, newData.AuthType, newData.Id)
		}
